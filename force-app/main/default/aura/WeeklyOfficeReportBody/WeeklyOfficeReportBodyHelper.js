({
    ...
    getContents : function(component){
        ...
        var initDate = component.get("v.oDate");
        console.log("O initDate : "+ initDate);
        ...
        self.apex(component, 'getOfficeReports', { reportDate : reportDate})
    ...
    callEditModal : function(component, recordId){
        var self = this,
            ...
            modalFooter;
        $A.createComponents([
            ["c:weeklyOfficeReportEditBody", { recordId : recordId }],
            ["c:weeklyOfficeReportEditFooter", { onsubmit : component.getReference("c.saveFromChild") }]
        ],
        function(components, status){
            if(status === "SUCCESS"){
                ...
                component.find('overlayLib').showCustomModal({
                    header : $A.get('$Label.c.WEEKLY_OFFICE_REPORT_EDIT_ISSUE'), // 'Edit Issue',
                    body : modalBody,
                    footer : modalFooter,
    ...
    callPreviewModal : function(component){
        ...
        $A.createComponents([
            ["c:weeklyOfficeReportPreviewBody", { reports : records }],
            ["c:weeklyOfficeReportPreviewFooter", { onsubmit : component.getReference("c.clickSendEmail") }]
        ],
        function(components, status){
            if(status === "SUCCESS"){
                modalBody = components[0];
                ...
                component.find('overlayLib').showCustomModal({
                    header : $A.get('$Label.c.WEEKLY_OFFICE_REPORT_PREVIEW'), // 'Preview',
                    body : modalBody,
                    footer : modalFooter,
                    showCloseButton : true,
