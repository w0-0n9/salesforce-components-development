({
    ...
    callKnoxSendEmail : function(component) {
        ...
        if(recipients.length < 1){
            error = true;
            var msg = $A.get("$Label.c.WEEKLY_OFFICE_REPORT_ERROR_RECIPIENTS");
        ...
        if(subject == ""){
            error = true;
            var msg = $A.get("$Label.c.WEEKLY_OFFICE_REPORT_ERROR_SUBJECT");
            self.showMyToast("error", msg);
        }
        if(comments == ""){
            error = true;
            var msg = $A.get("$Label.c.WEEKLY_OFFICE_REPORT_ERROR_COMMENT");
    ...
    sendKnoxEmail : function(component, emails, subject, mailBody){
        ...
        self.apex(component, 'sendReportKnoxEmail', {
            ...
            .then(function(result){
                ...
                if(result){
                    var msg = $A.get("$Label.c.WEEKLY_OFFICE_REPORT_MSG_SUCCESS");
                    self.showMyToast("success",msg);
                } else {
                    var msg = $A.get("$Label.c.WEEKLY_OFFICE_REPORT_ERROR_APEX_CONTROLLER");
                    self.showMyToast("error", msg);
                }
                component.set("v.showSpinner", false);
                //closes the modal or popover from the component
